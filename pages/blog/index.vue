<template>
  <section class="blog-block">
    <div class="container">
      <div
        class="sec-title text-center mb-3"
        data-aos="fade-up"
        data-aos-duration="1000"
      >
        <span class="title">BLOG & NEWS</span>
        <h2>We keep you always updated with</h2>
        <div class="divider">
          <span class="fa fa-mortar-board"></span>
        </div>
      </div>
      <div class="row">
        <div class="col-lg-4 col-md-6 mb-5">
          <div class="post" data-aos="fade-up" data-aos-duration="1000">
            <img class="img-fluid" src="/images/blog_2.jpg" alt="Blog" />
            <div class="post_inner p-3">
              <p class="mb-1 font-weight-bold">13th March,2019</p>
              <h5 class="font-weight-bold">Contrary to popular belief</h5>
              <p>
                Lorem Ipsum is not simply random text. It has roots in a piece
                of classical Latin literature from 45 BC, making it over 2000
                years old.
              </p>
              <div class="comment d-flex">
                <span class="mr-3"
                  ><i class="fa fa-user mr-3 color-orange"></i>Admin</span
                >
                <span class="mr-3"
                  ><i class="fa fa-comments mr-3 color-orange"></i
                  >Comments</span
                >
              </div>
              <a class="btn color-orange read-more mt-3" href="/blog/id"
                >Read More</a
              >
            </div>
          </div>
        </div>

        <div class="col-lg-4 col-md-6 mb-5">
          <div class="post" data-aos="fade-up" data-aos-duration="1000">
            <img class="img-fluid" src="/images/blog_3.jpg" alt="Blog" />
            <div class="post_inner p-3">
              <p class="mb-1 font-weight-bold">13th March,2019</p>
              <h5 class="font-weight-bold">Contrary to popular belief</h5>
              <p>
                Lorem Ipsum is not simply random text. It has roots in a piece
                of classical Latin literature from 45 BC, making it over 2000
                years old.
              </p>
              <div class="comment d-flex">
                <span class="mr-3"
                  ><i class="fa fa-user mr-3 color-orange"></i>Admin</span
                >
                <span class="mr-3"
                  ><i class="fa fa-comments mr-3 color-orange"></i
                  >Comments</span
                >
              </div>
              <nuxt class="btn color-orange read-more mt-3" href="/blog/id"
                >Read More</nuxt
              >
            </div>
          </div>
        </div>

        <div class="col-lg-4 col-md-6 mb-5">
          <div class="post" data-aos="fade-up" data-aos-duration="1000">
            <img class="img-fluid" src="/images/blog_2.jpg" alt="Blog" />
            <div class="post_inner p-3">
              <p class="mb-1 font-weight-bold">13th March,2019</p>
              <h5 class="font-weight-bold">Contrary to popular belief</h5>
              <p>
                Lorem Ipsum is not simply random text. It has roots in a piece
                of classical Latin literature from 45 BC, making it over 2000
                years old.
              </p>
              <div class="comment d-flex">
                <span class="mr-3"
                  ><i class="fa fa-user mr-3 color-orange"></i>Admin</span
                >
                <span class="mr-3"
                  ><i class="fa fa-comments mr-3 color-orange"></i
                  >Comments</span
                >
              </div>
              <a class="btn color-orange read-more mt-3" href="/blog/id"
                >Read More</a
              >
            </div>
          </div>
        </div>

        <div
          class="col-lg-4 col-md-6 mb-lg-0 mb-sm-5 mb-xl-0 mb-sm-5 mb-md-0 mb-5"
        >
          <div class="post" data-aos="fade-up" data-aos-duration="1000">
            <img class="img-fluid" src="/images/blog_3.jpg" alt="Blog" />
            <div class="post_inner p-3">
              <p class="mb-1 font-weight-bold">13th March,2019</p>
              <h5 class="font-weight-bold">Contrary to popular belief</h5>
              <p>
                Lorem Ipsum is not simply random text. It has roots in a piece
                of classical Latin literature from 45 BC, making it over 2000
                years old.
              </p>
              <div class="comment d-flex">
                <span class="mr-3"
                  ><i class="fa fa-user mr-3 color-orange"></i>Admin</span
                >
                <span class="mr-3"
                  ><i class="fa fa-comments mr-3 color-orange"></i
                  >Comments</span
                >
              </div>
              <a class="btn color-orange read-more mt-3" href="/blog/id"
                >Read More</a
              >
            </div>
          </div>
        </div>

        <div class="col-lg-4 col-md-6 mb-lg-0 mb-xl-0 mb-sm-5 mb-md-0 mb-5">
          <div class="post" data-aos="fade-up" data-aos-duration="1000">
            <img class="img-fluid" src="/images/blog_2.jpg" alt="Blog" />
            <div class="post_inner p-3">
              <p class="mb-1 font-weight-bold">13th March,2019</p>
              <h5 class="font-weight-bold">Contrary to popular belief</h5>
              <p>
                Lorem Ipsum is not simply random text. It has roots in a piece
                of classical Latin literature from 45 BC, making it over 2000
                years old.
              </p>
              <div class="comment d-flex">
                <span class="mr-3"
                  ><i class="fa fa-user mr-3 color-orange"></i>Admin</span
                >
                <span class="mr-3"
                  ><i class="fa fa-comments mr-3 color-orange"></i
                  >Comments</span
                >
              </div>
              <a class="btn color-orange read-more mt-3" href="/blog/id"
                >Read More</a
              >
            </div>
          </div>
        </div>

        <div class="col-lg-4 col-md-6 mb-xl-0 mb-md-0">
          <div class="post" data-aos="fade-up" data-aos-duration="1000">
            <img class="img-fluid" src="/images/blog_3.jpg" alt="Blog" />
            <div class="post_inner p-3">
              <p class="mb-1 font-weight-bold">13th March,2019</p>
              <h5 class="font-weight-bold">Contrary to popular belief</h5>
              <p>
                Lorem Ipsum is not simply random text. It has roots in a piece
                of classical Latin literature from 45 BC, making it over 2000
                years old.
              </p>
              <div class="comment d-flex">
                <span class="mr-3"
                  ><i class="fa fa-user mr-3 color-orange"></i>Admin</span
                >
                <span class="mr-3"
                  ><i class="fa fa-comments mr-3 color-orange"></i
                  >Comments</span
                >
              </div>
              <a class="btn color-orange read-more mt-3" href="/blog/id"
                >Read More</a
              >
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
export default {
  name: "Blog-Page"
};
</script>

<style></style>
